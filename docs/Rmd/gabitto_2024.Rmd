...
```{r Filter-cell-type, message=FALSE, warning=FALSE}
seu %>% filter_cell_types_in_seu_object(n_cell_min = 500, dry_run = TRUE, verbose = TRUE)
```
Note: in this test dataset there is a single cell type with more than 500 cells so this function has no effect.
...
```{r compute-correlations-by-cell-type-and-chunk-size, message=FALSE, warning=FALSE}
...
```{r compute-correlations-by-chunk-size, message=FALSE, warning=FALSE}
...
```
Note: in this test dataset there is a single cell type with more than 500 cells so computing correlations by cell type first is not meaningful.
